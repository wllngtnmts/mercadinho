...
if(document.querySelector('.btnPermSen')){
    ...
    tablePerm.addEventListener('click',(event)=>{
        if(event.target.className == 'btnPermSen'){
            estruturaPopup('permitionSen', '.popupPermSen', event.target.dataset.matricula, event.target.dataset.id);
        }
    });
}
...
if(document.querySelector('.btnPerm')){
    ...
    tablePerm.addEventListener('click',(event)=>{
        if(event.target.className == 'btnPerm'){
            estruturaPopup('permition','.popupPerm', event.target.dataset.matricula, event.target.dataset.id);
        }
    });
}